...
class RGPayPalUpgrade{

    public static function set_version_info($version_info){
        if ( function_exists('set_site_transient') )
            set_site_transient("gforms_paypal_version", $version_info, 60*60*12);
        else
            set_transient("gforms_paypal_version", $version_info, 60*60*12);
    }

    public static function check_update($plugin_path, $plugin_slug, $plugin_url, $offering, $key, $version, $option){
        $version_info = function_exists('get_site_transient') ? get_site_transient("gforms_paypal_version") : get_transient("gforms_paypal_version");
