...
class GFPayPal {
    ...
    public static function send_to_paypal($confirmation, $form, $entry, $ajax){
        ...
        if(!$config)
            ...
            $config = $config[0]; //using first paypal feed (only one paypal feed per form is supported)

        // ignore requests when paypal conditional is active and condition is not met
        if(!self::has_paypal_condition($form, $config)) return $confirmation;
    ...
    private static function verify_paypal_ipn(){
        ...
        $request = new WP_Http();
        $response = $request->post($url, array("sslverify" => false, "ssl" => true, "body" => $req));
