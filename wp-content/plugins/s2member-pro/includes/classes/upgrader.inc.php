		...
		class c_ws_plugin__s2member_pro_upgrader
			{
				...
				*/
				public static function /* Pro Upgrader. */ upgrade ()
					{
						...
						if (!empty ($_POST["ws_plugin__s2member_pro_upgrade"]) && ($nonce = (string)$_POST["ws_plugin__s2member_pro_upgrade"]) && wp_verify_nonce ($nonce, "ws-plugin--s2member-pro-upgrade") && ($_p = c_ws_plugin__s2member_utils_strings::trim_deep (stripslashes_deep ($_POST))))
							{
								if (@set_time_limit (0) !== "nill" && @ini_set ("memory_limit", apply_filters ("admin_memory_limit", WP_MAX_MEMORY_LIMIT)) !== "nill" && @ini_get ("memory_limit") === apply_filters ("admin_memory_limit", WP_MAX_MEMORY_LIMIT))
									{
										if (!empty ($_p["ws_plugin__s2member_pro_upgrade_username"]) && !empty ($_p["ws_plugin__s2member_pro_upgrade_password"]) && is_array ($cs_s2member_pro_upgrade = maybe_unserialize (c_ws_plugin__s2member_utils_urls::remote (add_query_arg (urlencode_deep (array ("cs_s2member_pro_upgrade" => array ("username" => (string)$_p["ws_plugin__s2member_pro_upgrade_username"], "password" => (string)$_p["ws_plugin__s2member_pro_upgrade_password"], "version" => WS_PLUGIN__S2MEMBER_PRO_VERSION))), c_ws_plugin__s2member_readmes::parse_readme_value ("Pro Module / Auto-Update URL", dirname (dirname (dirname (__FILE__))) . "/readme.txt"))))) && !empty ($cs_s2member_pro_upgrade["zip"]) && !empty ($cs_s2member_pro_upgrade["ver"]))
											{
												...
												if (WP_Filesystem (c_ws_plugin__s2member_pro_upgrader::$credentials, ($plugins_dir = $_plugins_dir = dirname (dirname (dirname (dirname (__FILE__)))))) && ($plugins_dir = rtrim ($wp_filesystem->find_folder ($plugins_dir), "/")) && ($plugin_dir = rtrim ($wp_filesystem->find_folder ($_plugin_dir = dirname (dirname (dirname (__FILE__)))), "/")))
													{
														if (($tmp_zip = wp_unique_filename ($_plugins_dir, basename ($plugin_dir) . ".zip")) && ($_tmp_zip = $_plugins_dir . "/" . $tmp_zip) && ($tmp_zip = $plugins_dir . "/" . $tmp_zip) && $wp_filesystem->put_contents ($tmp_zip, c_ws_plugin__s2member_utils_urls::remote ($cs_s2member_pro_upgrade["zip"]), FS_CHMOD_FILE))
															{
										...
										else if (!empty ($cs_s2member_pro_upgrade) && $cs_s2member_pro_upgrade === /* Forbidden? */ "403 Forbidden")
											{
												c_ws_plugin__s2member_pro_upgrader::$error = "Upgrade failed. Invalid Username/Password ( please try again ).";
											}
										else if (!empty ($cs_s2member_pro_upgrade) && $cs_s2member_pro_upgrade === "503 Service Unavailable")
											{
